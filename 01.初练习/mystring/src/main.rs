...
impl MyString {
    fn push_str(&mut self, s: &str) {
        match self {
            Self::Inline(v) => {
                let inline_len = v.len();
                let len = inline_len + s.len();

                if len > MINI_STRING_MAX_LEN {
                    let mut owned = v.deref().to_string();
                    owned.push_str(s);
                    *self = Self::Standard(owned)
                } else {
                    v.data[inline_len..len].copy_from_slice(s.as_bytes());
                    v.len = len as u8;
                }
            }
            Self::Standard(v) => v.push_str(s),
        }
    }
}
...
fn main() {
    ...
    let s2: MyString = "这是一个很长很长的超过了三十个字节的很长很长的字符串".into();
    // debug 输出
    println!("s1: {:?}, s2: {:?}", s1, s2);
    // display 输出
    println!(
